...
def get_data():
    ...
    if response.status_code == 200:
        ...
        df = pd.read_excel(excel_data, sheet_name=None)  # sheet_name=None pour lire toutes les feuilles
    
        # Créer le dossier 'data/raw' si nécessaire
        output_dir = "data/raw"
        os.makedirs(output_dir, exist_ok=True)  # Crée le dossier si ce n'est pas déjà fait
        ...
        with open(output_csv, 'w', newline='', encoding='utf-8') as csv_file:
            df_all_sheets = pd.concat(df.values(), ignore_index=True)  # Si plusieurs feuilles, les concaténer
