...
def afficher_map(colonne):
    ...
    missing_from_geojson = [
        country for country in csv_countries if country not in geojson_countries
    ]
    missing_from_csv = [
        country for country in geojson_countries if country not in csv_countries
    ]

    print(
        "Pays présents dans le CSV mais absents dans le GeoJSON:", missing_from_geojson
    )
    ...
    if colonne == "Total Damage Adjusted 000 US":
        damage_counts = (
            data.groupby("Country")[colonne]
            .sum()
            .reset_index(name="Total Damage Adjusted")
        )
